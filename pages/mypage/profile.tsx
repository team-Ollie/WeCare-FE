  ...
  return (
    <FlexBox direction="col">
      ...
      <FlexBox className="w-full px-6 py-4 justify-between">
        ...
        <div className="h4 text-grey-700">{data?.result.nickname}</div>
      </FlexBox>
      <FlexBox className="w-full px-6 py-4 justify-between">
        <div className="h4">아이디</div>
        <div className="h4 text-grey-700">{data?.result.loginId}</div>
      </FlexBox>
      <Divider height={8} />
      <FlexBox direction="col" className="w-full items-end gap-2 px-6">
        <div
          className="underline h5 text-grey-500 pt-4 pb-2"
          onClick={() => router.push("/mypage/nickname")}
        >
          닉네임 변경
        </div>
        <div
          className="underline h5 text-grey-500 py-2"
